...
A document-first delivery SOP that takes a product idea to a shipped feature through four artifacts (`requirements` → `spec` → `plan` → `reviews`) and a chunked TDD implementation. Every architectural or behavioral change is forced back through the spec, so the docs and the code never drift.
...
- Implicit: user describes a product idea / feature and expects a structured handoff from Idea → delivery instead of jumping straight into code; user references `docs/requirements.md`, `docs/spec.md`, `docs/plan.md` or asks to "固化/制定/审查" these.
...
```
Idea → requirements.md → spec.md → [spec review] → plan.md → [plan review]
...
docs/requirements.md           需求说明（产品目标、用户、场景、功能/非功能、非目标、成功标准）
docs/spec.md                   产品与技术规格（范围、信息架构、数据模型、状态、领域行为、视觉、组件边界、验收标准）
docs/plan.md                   实施计划（Chunk → Task → Files/Interfaces/Test cases/Steps，含 DoD）
docs/reviews/spec-review.md    规格独立审查记录
docs/reviews/plan-review.md    计划独立审查记录
docs/reviews/code-review.md    代码独立审查记录（可选：acceptance.md）
```
...
7. **成功标准** — measurable: usability, resilience, test coverage, build/lint green.
...
10. **验收标准** — numbered, each mappable to an automated test or manual check.
...
### Stage 2b — Spec independent review

A separate, read-only reviewer checks the spec against requirements. Record findings in `docs/reviews/spec-review.md` using **阻断 / 重要 / 次要** tiers. Fix all 阻断 + 重要, re-review, then mark status `已通过独立审查，可进入实施计划`. Only then proceed. Reviewer stays read-only; the author applies fixes.

## Stage 3 — Plan (`docs/plan.md`)

Decompose the spec into Chunks → Tasks. Order so each task is independently verifiable and testable.

Per `plan.md` structure:

- **Goal** + **全局约束** (tech stack lock, persistence lock, non-goal lock, test-first lock, stop-on-spec-conflict).
...
Commit prefix: `docs: 制定并审查 <feature> 实施计划`.

### Stage 3b — Plan independent review

Same read-only reviewer pattern as Stage 2b, recorded in `docs/reviews/plan-review.md`. Verify: test harness won't miscollect unrelated tests; cleanup scope is complete; every spec acceptance criterion has an executing task; deployment/release ordering pins the exact commit. Fix all 阻断 + 重要 before implementing.
...
Read-only reviewer audits the full diff from the spec commit to HEAD against `AGENTS.md`, `docs/spec.md`, `docs/plan.md`, and the test/build output. Tiers again **阻断 / 重要 / 次要**. Record in `docs/reviews/code-review.md`. The author fixes all 阻断 + 重要 and adds regression tests; reviewer re-verifies read-only. No blocking items may remain open. If there are no code issues, commit `docs: 记录独立代码审查` — never manufacture a fake `fix:`.
...
2. Create `docs/reviews/acceptance.md` mapping each spec acceptance criterion to its automated test / static check / manual evidence.
...
| 2 Spec | `docs/spec.md` | Spec review passes (阻断+重要 closed) |
| 3 Plan | `docs/plan.md` | Plan review passes (阻断+重要 closed) |
| 4 Implementation | Chunked TDD commits | Each task's targeted test green |
...
3. Update `plan.md` with the affected Chunk(s)/Task(s); skip already-done tasks.
